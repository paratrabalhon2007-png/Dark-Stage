...
const App: React.FC = () => {
      ...
      <div className="min-h-screen flex items-center justify-center px-4 bg-[#0a0a0a]">
        <div className="w-full max-w-md animate-fadeIn">
          <div className="text-center mb-10">
            <div className="w-32 h-32 bg-white rounded-[2.5rem] flex items-center justify-center mx-auto mb-8 shadow-2xl">
  ...
  const renderAdmin = () => (
    <div className="max-w-4xl mx-auto px-4 py-8 animate-fadeIn">
      <div className="flex items-center justify-between mb-10">
        <h1 className="text-xl font-black uppercase italic text-white">Gestão de Alunos</h1>
      ...
      <div className="bg-[#1a1a1a] rounded-3xl border border-white/5 overflow-hidden">
        <table className="w-full text-left">
          <thead className="bg-white/5 text-[9px] font-black uppercase text-gray-500">
            <tr><th className="px-6 py-5">Nome</th><th className="px-6 py-5">E-mail</th><th className="px-6 py-5 text-right">Ação</th></tr>
          </thead>
        ...
        <div className="fixed inset-0 z-50 bg-black/95 flex items-center justify-center p-4">
          <div className="w-full max-w-md bg-[#1a1a1a] border border-white/10 rounded-3xl p-8 shadow-2xl">
            ...
            <div className="space-y-5">
              <input type="text" placeholder="Nome" value={newStudent.name} onChange={e => setNewStudent({...newStudent, name: e.target.value})} className="w-full bg-[#0f0f0f] border border-white/10 rounded-xl px-4 py-4 text-sm text-white outline-none" />
              <input type="email" placeholder="E-mail" value={newStudent.email} onChange={e => setNewStudent({...newStudent, email: e.target.value})} className="w-full bg-[#0f0f0f] border border-white/10 rounded-xl px-4 py-4 text-sm text-white outline-none" />
              <input type="password" placeholder="Senha" value={newStudent.password} onChange={e => setNewStudent({...newStudent, password: e.target.value})} className="w-full bg-[#0f0f0f] border border-white/10 rounded-xl px-4 py-4 text-sm text-white outline-none" />
            </div>
            <div className="mt-10 flex gap-4">
              ...
              <button onClick={addStudent} className="flex-1 bg-white text-black py-4 rounded-xl text-[10px] font-black uppercase">Salvar</button>
            </div>
          </div>
        </div>
      )}
  ...
  return (
    <div className="min-h-screen bg-[#0f0f0f] text-white font-sans">
      <nav className="sticky top-0 z-50 bg-[#0f0f0f]/90 backdrop-blur-xl border-b border-white/5 px-6 py-5 flex items-center justify-between">
        ...
        <div className="flex items-center gap-6">
          {currentUser === 'admin@darkstage.com' && (
            <button onClick={() => setIsAdminMode(!isAdminMode)} className="flex items-center gap-2 text-white bg-white/5 px-4 py-2 rounded-xl border border-white/10 hover:bg-white/10">
      ...
      <footer className="fixed bottom-0 left-0 w-full p-6 text-center pointer-events-none">
